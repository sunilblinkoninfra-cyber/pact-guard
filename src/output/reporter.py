...
def build_json_report(
    ...
    numbered_findings = []
    enriched_map = {}
    for ef in ai_enrichment.get("enriched_findings", []):
        enriched_map[ef.get("rule_id", "")] = ef

    for i, f in enumerate(findings, start=1):
        fd = f.to_dict(i)
        enrich = enriched_map.get(f.rule_id, {})
        if enrich:
...
def render_cli(report: Dict[str, Any], color: bool = True) -> str:
    ...
    lines.append("")
    lines.append("╔══════════════════════════════════════════════════════════════╗")
    lines.append("║          PACT SENTINEL — Security Analysis Report            ║")
    lines.append("╚══════════════════════════════════════════════════════════════╝")
...
def render_markdown(report: Dict[str, Any]) -> str:
    ...
    lines.append("# 🛡️ Pact Sentinel — Security Analysis Report")
    lines.append(f"")
    lines.append(f"**File:** `{report.get('analyzed_file', 'unknown')}`  ")
    ...
    lines.append("")
    lines.append("*Generated by [pact-sentinel](https://github.com/your-org/pact-sentinel)*")
    return "\n".join(lines)
        ...
        "runs": [{
            "tool": {
                "driver": {
                    "name": "pact-sentinel",
                    "version": "1.0.0",
                    "informationUri": "https://github.com/your-org/pact-sentinel",
                    "rules": rules,
                }
            },
            "results": results,
